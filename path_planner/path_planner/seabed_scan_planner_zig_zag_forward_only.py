...
class SeabedScanPlannerZigZagForwardOnly(SeabedScanPlanner):
    def __init__(self):
        super().__init__()
    ...
    def build_raster_waypoints(
        ...
        first_row_y = y_offsets[0]
        waypoints.append(to_world(0.0, first_row_y))
        if width > 0.0:
            waypoints.append(to_world(width, first_row_y))

        for row_idx, y_offset in enumerate(y_offsets[1:], start=1):
            x_target = 0.0 if row_idx % 2 == 1 else width
            waypoints.append(to_world(x_target, y_offset))
...
def main(args=None):
    ...
    node = SeabedScanPlannerZigZagForwardOnly()
    try:
        rclpy.spin(node)
